...
from easycalib.lib.geometric_vision import solve_pnp, solve_pnp_ransac, zhangzhengyou_caliberate, rtvec_to_matrix, calculate_similarity, render_debug_coordinates, left_handed_coord2right_handed_coord
...
def analyze_ndds_dataset(
	...
	all_kpt_projs_pred = []
	all_kpt_gt_pos = []
	data_path = found_ndds_dataset_data[0]["data_path"]
	gt_cam_ext = dream.utilities.load_camera_gt_ext(
		data_path,
		dream_network.manipulator_name,
	)
	...
	if pnp_analysis:
		...
		for kp_projs_est, kp_projs_gt, kp_pos_gt in zip(
			...
			all_kpt_projs_pred.append(kp_projs_est_pnp)
			all_kpt_gt_pos.append(kp_pos_gt_pnp)

			pnp_retval, translation, quaternion, _ = dream.geometric_vision.solve_pnp(
		...
		# ! Conduct PnP for all_
		all_kpt_projs_pred = np.vstack(all_kpt_projs_pred)
		all_kpt_gt_pos = np.vstack(all_kpt_gt_pos)

		pnp_retval, translation, quaternion, reproj_err = dream.geometric_vision.solve_pnp(
			all_kpt_gt_pos, all_kpt_projs_pred, camera_K
		)
						
		if pnp_retval:
			pred_cam_ext = rtvec_to_matrix(
				translation, quaternion
			)
			trans_err, rot_err = calculate_similarity(
				pred_cam_ext, gt_cam_ext
			)

			logger.info(
				f"The gt_transforms matrix is \n {gt_cam_ext} \n the predicted matrix is {pred_cam_ext} \n"
			)
			logger.info(f"the trans_error is {trans_err}, and the rot_err is {rot_err}")
		else:
			trans_err = np.array([-999.99, -999.99, -999.99])
			rot_err = np.array([-999.99])
			reproj_err = np.array(-999.99)
			pred_cam_ext = np.eye(4)

		all_pnp_path = os.path.join(output_dir, "caliberate_res.pkl")
		save_obj = {
		"avg_trans_err": trans_err.tolist(),
		"avg_rot_err": rot_err.item(),
		"avg_reprojection_err": reproj_err.item(),
		"pnp_transform_predicted_mats": pred_cam_ext.tolist(),
		"gt_local_to_world_matrix": gt_cam_ext.tolist(),
		"camera_K": camera_K.tolist(),
		}
		# logger.info(f"avg_trans_err:{trans_err.tolist()}, avg_rot_err: {rot_err.item()}, reproj_err: {reproj_err.item()}, pred_cam_ext:{pred_cam_ext.tolist()}")
		pickle.dump(save_obj, open(all_pnp_path, "wb"))
